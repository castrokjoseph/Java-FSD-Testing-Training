...
select name as film_name, length_min as filmlength from films where length_min > (select avg(length_min) from films);
...
select f.name as film_name,max(s.id),min(s.id) from screenings s inner join films f on s.film_id = f.id

group by f.name;

/* 3) solution*/

select f.name as film_name, count(s.id) as No_of_screening from screenings s inner join films f on s.film_id = f.id group by s.film_id;